...
- **Bilder:** Galerie mit Vollbild-Ansicht. Fotos werden beim Hochladen **automatisch auf max. 1920 px verkleinert** (JPEG; spart Browserspeicher — ein 9-MB-Handyfoto wird zu wenigen hundert KB; kleine Bilder und GIF/SVG bleiben unverändert). Dazu **🔎 Texterkennung (OCR)**: Der 🔎-Knopf auf einem Bild erkennt den Folientext (deutsch+englisch; der erste Lauf lädt einmalig Sprachdaten, online nötig). Erkannter Text wird am Bild gespeichert (grüne Markierung), ist über das Suchfeld der Galerie **durchsuchbar** und lässt sich per Klick **als Notiz übernehmen** — so werden abfotografierte Folien wiederauffindbar.
